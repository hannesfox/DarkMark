            ...
            if filename.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.tiff')):
                ...
                try:
                    ...
                    if template_img is None:
                        print(
                            f"WARNUNG: Konnte Benutzer-Bild {filename} nicht als Template laden (cv2.imread gab None zurück).")
                        continue
        ...
        try:
            ...
            with fitz.open(self.input_path) as doc:
                ...
                if total_redactions > 0:
                    ...
                    print(
                        f"DEBUG: RedactionTask: Saved {os.path.basename(self.output_path)} with {total_redactions} redactions.")
                else:
                    print(
                        f"DEBUG: RedactionTask: No redactions found for {os.path.basename(self.input_path)}, not saving.")
    ...
    def run(self):
        try:
            ...
            with fitz.open(self.original_pdf_path) as doc:
                ...
                print(
                    f"DEBUG: PreviewRedactionTask: Saved temporary {os.path.basename(temp_output_path)} with {total_redactions} redactions.")
...
class DrawingCanvas(QLabel):
    ...
    def wheelEvent(self, event: QWheelEvent):
        ...
        self.main_app.state["template_canvas_pan_offset"] = pos - image_pos_before_zoom * self.main_app.state[
            "template_canvas_zoom"]
    ...
    def mousePressEvent(self, event: QMouseEvent):
        ...
        if event.button() == Qt.MouseButton.MiddleButton:
            ...
            self.main_app.state["template_canvas_end_point_orig"] = self.main_app.state[
                "template_canvas_start_point_orig"]
    ...
    def mouseReleaseEvent(self, event: QMouseEvent):
        ...
        if self.main_app.state["template_canvas_panning"] and event.button() == Qt.MouseButton.MiddleButton:
            ...
            new_rect = QRectF(self.main_app.state["template_canvas_start_point_orig"],
                              self.main_app.state["template_canvas_end_point_orig"]).normalized()
...
class DarkMarkApp(QMainWindow):
    ...
    def update_ui(self):
        ...
        if is_in_redaction_mode:
            ...
            current_display_paths_list = self.state["preview_pdf_paths"] if is_in_preview_mode else self.state[
                "original_pdf_paths"]
            if current_display_paths_list:
                self.pdf_info_label.setText(
                    f"PDF: {self.state['current_pdf_index'] + 1} / {len(current_display_paths_list)}")
            else:
            ...
            self.prev_pdf_button.setEnabled(
                bool(not is_processing and self.state["current_pdf_index"] > 0 and current_display_paths_list))
            self.next_pdf_button.setEnabled(
                bool(not is_processing and current_display_paths_list and self.state["current_pdf_index"] < len(
                    current_display_paths_list) - 1))
            ...
            self.redact_preview_button.setEnabled(
    ...
    def _clear_temp_preview_files(self):
        ...
        if self.current_temp_preview_dir and os.path.exists(self.current_temp_preview_dir):
            try:
                ...
                print(
                    f"WARNUNG: Fehler beim Löschen des temporären Vorschau-Verzeichnisses {self.current_temp_preview_dir}: {e}")
        self.current_temp_preview_dir = None
    ...
    def reset_template_canvas(self):
        ...
        print(
            f"DEBUG: reset_template_canvas called (clearing template_canvas_pdf_path: {self.state['template_canvas_pdf_path']}).")
    ...
    def prev_pdf(self):
        current_paths = self.state["preview_pdf_paths"] if self.state["is_in_preview_mode"] else self.state[
            "original_pdf_paths"]
        ...
        if self.preview_batch_processed >= self.preview_batch_total:
            ...
            if self.state["preview_pdf_paths"]:
                ...
                self.status_label.setText(
                    f"Vorschau-Schwärzung abgeschlossen. {len(self.state['preview_pdf_paths'])} Dateien zum Ansehen bereit.")
            else:
                self.status_label.setText(
                    "Vorschau-Schwärzung abgeschlossen. Keine PDFs generiert (oder alle fehlgeschlagen).")
                self.clear_all_docs_except_templates()
    ...
    def go_to_original_mode(self):
        ...
        if self.state["original_pdf_paths"]:
            self.state["current_pdf_index"] = 0
            try:
                self._load_pdf_into_state(self.state["original_pdf_paths"][self.state["current_pdf_index"]],
                                          "original_doc")
    ...
    def reset_template_canvas_view_and_rects(self):
        ...
        self.state["template_canvas_pan_offset"] = QPointF(
            (self.template_canvas.width() - self.state["template_canvas_pixmap"].width() * self.state[
                "template_canvas_zoom"]) / 2,
            (self.template_canvas.height() - self.state["template_canvas_pixmap"].height() * self.state[
                "template_canvas_zoom"]) / 2
        )
        self.template_canvas.update()
        self.undo_template_button.setEnabled(False)
